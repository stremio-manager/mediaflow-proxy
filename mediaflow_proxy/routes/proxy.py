...
@proxy_router.get("/mpd/ffmpeg-segment/{stream_id}/{filename}")
async def ffmpeg_segment_proxy(
    request: Request,
    stream_id: str,
    filename: str,
):
    """
    Serves a segment or playlist generated by FFmpeg.
    """
    from mediaflow_proxy.handlers import handle_ffmpeg_segment

    return await handle_ffmpeg_segment(request, stream_id, filename)

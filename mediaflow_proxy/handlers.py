...
async def handle_ffmpeg_segment(
    request: Request,
    stream_id: str,
    filename: str,
) -> Response:
    ...
    file_path = os.path.join(settings.ffmpeg_temp_dir, stream_id, filename)

    if not os.path.exists(file_path):
        raise HTTPException(status_code=404, detail="Segment not found")

    # Touch the stream to keep it alive
    ffmpeg_manager.touch_stream(stream_id)
